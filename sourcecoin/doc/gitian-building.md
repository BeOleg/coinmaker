...
*Setup instructions for a gitian build of CM_UppercaseCoinName using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the CM_UppercaseCoinName
...
- [Building CM_UppercaseCoinName](#building-CM_LowercaseCoinName)
...
Clone the git repositories for CM_LowercaseCoinName and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/CM_LowercaseCoinName/CM_LowercaseCoinName
```
...
In [doc/release-process.md](release-process.md) in the CM_LowercaseCoinName repository under 'Fetch and build inputs'.
...
Building CM_UppercaseCoinName
----------------

To build CM_UppercaseCoinName (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the CM_LowercaseCoinName repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/CM_LowercaseCoinName/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/CM_LowercaseCoinName/CM_LowercaseCoinName
...
For example:
```bash
URL=https://github.com/laanwj/CM_LowercaseCoinName.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit CM_LowercaseCoinName=${COMMIT} --url CM_LowercaseCoinName=${URL} ../CM_LowercaseCoinName/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit CM_LowercaseCoinName=${COMMIT} --url CM_LowercaseCoinName=${URL} ../CM_LowercaseCoinName/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/CM_LowercaseCoinName-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/CM_LowercaseCoinName-build.assert
```
...
[CM_LowercaseCoinName/gitian.sigs](https://github.com/CM_LowercaseCoinName/gitian.sigs/) repository, or
