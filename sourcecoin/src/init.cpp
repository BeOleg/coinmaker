...
void Shutdown()
{
    ...
    RenameThread("CM_LowercaseCoinName-shutoff");
    ...
    if (pwalletMain)
        bitdb.Flush(false);
    GenerateCM_UppercaseCoinNames(false, NULL, 0);
#endif
    StopNode();
...
std::string HelpMessage(HelpMessageMode hmm)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: CM_LowercaseCoinName.conf)") + "\n";
    if (hmm == HMM_CM_AllCapsCoinNameD)
    {
#if !defined(WIN32)
    ...
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: CM_LowercaseCoinNamed.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: CM_Port or testnet: 1CM_Port)") + "\n";
    ...
    if (hmm == HMM_CM_AllCapsCoinName_QT)
        strUsage += ", qt";
    strUsage += ".\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: CM_RPCPort or testnet: 1CM_RPCPort)") + "\n";
    strUsage += "  -rpcallowip=<ip>       " + _("Allow JSON-RPC connections from specified IP address") + "\n";
    strUsage += "  -rpcthreads=<n>        " + _("Set the number of threads to service RPC calls (default: 4)") + "\n";

    strUsage += "\n" + _("RPC SSL options: (see the CM_UppercaseCoinName Wiki for SSL setup instructions)") + "\n";
...
{
    RenameThread("CM_LowercaseCoinName-loadblk");
...
/** Initialize CM_LowercaseCoinName.
...
#define PROCESS_DEP_ENABLE 0x00000001
...
#endif
    // Make sure only a single CM_UppercaseCoinName process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CM_UppercaseCoinName Core is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("CM_UppercaseCoinName version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of CM_UppercaseCoinName") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart CM_UppercaseCoinName to complete") << "\n";
    ...
    if (pwalletMain)
        GenerateCM_UppercaseCoinNames(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", -1));
#endif
