...
Value GetNetworkHashPS(int lookup, int height) {
    ...
    if (lookup <= 0)
        lookup = pb->nHeight % CM_RetargetBlocks + 1;
...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or CM_LowercaseCoinName.conf setting gen)\n"
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    if (fGenerate && Params().NetworkID() == CChainParams::REGTEST)
    {
        ...
        while (nHeight < nHeightEnd)
        {
            ...
            {
                nHeightLast = nHeight;
                GenerateCM_UppercaseCoinNames(fGenerate, pwalletMain, 1);
            }
            MilliSleep(1);
            {   // Don't keep cs_main locked
                LOCK(cs_main);
    ...
    else // Not -regtest: start generate thread, return immediately
    {
        ...
        GenerateCM_UppercaseCoinNames(fGenerate, pwalletMain, nGenProcLimit);
    }
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "CM_UppercaseCoinName is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "CM_UppercaseCoinName is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.CM_LowercaseCoinName.it/wiki/BIP_0022 for full specification.\n"
