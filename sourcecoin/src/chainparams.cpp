...
public:
    CMainParams() {
        ...
        nRPCPort = CM_RPCTestnetPort;
        bnProofOfWorkLimit = CBigNum(~uint256(0) >> 32);
        nSubsidyHalvingInterval = CM_HalvingIntervalBlocks;
        ...
        const char* pszTimestamp = "CM_GenesisHeadline";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("CM_GenesisPubkey") << OP_CHECKSIG;
        genesis.vtx.push_back(txNew);
        genesis.hashPrevBlock = 0;
        genesis.hashMerkleRoot = genesis.BuildMerkleTree();
        genesis.nVersion = 1;
        genesis.nTime    = CM_GenesisTimecode;
        genesis.nBits    = 0x1d00ffff;
        genesis.nNonce   = CM_GenesisNonce;

        hashGenesisBlock = genesis.GetHash();
        assert(hashGenesisBlock == uint256("0xCM_GenesisHash"));
        assert(genesis.hashMerkleRoot == uint256("0xCM_GenesisMerkleHash"));

        base58Prefixes[PUBKEY_ADDRESS] = list_of(CM_AddressVersion);
