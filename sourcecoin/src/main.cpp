...
#if defined(NDEBUG)
# error "CM_UppercaseCoinName cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "CM_UppercaseCoinName Signed Message:\n";
...
int64_t GetBlockValue(int nHeight, int64_t nFees)
{
    ...
    int64_t b1value = CM_BlockOneValue;
...
static int64_t nTargetSpacing = CM_TargetTimespanSeconds;
...
void ThreadScriptCheck() {
    RenameThread("CM_LowercaseCoinName-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
   ...
   //
    // This rule applies to all CM_UppercaseCoinName blocks whose timestamp is after March 15, 2012, 0:00 UTC.
    //
    ...
    // as CM_UppercaseCoinName, namely the one suggested below
    // This rule was originally applied all blocks whose timestamp was after March 15, 2012, 0:00 UTC.
    // Now that the whole chain is irreversibly beyond that time it is applied to all blocks except the
    // two in the chain that violate it. This prevents exploiting the issue against nodes in their
    // initial block download.
    ...
    if (block.vtx[0].GetValueOut() > (GetBlockValue(pindex->nHeight, nFees) - fallbackReduction))
    {
        ...
        for (unsigned int i = 1; i < block.vtx[0].vout.size(); i++)
        {
			...
			if (ExtractDestination(block.vtx[0].vout[i].scriptPubKey, txaddress))
			{
				...
				CCM_UppercaseCoinNameAddress addr(txaddress);
				if (addr.IsValid())
				{
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    if ((chainActive.Height() % (CM_RetargetBlocks*10)) == 0 || (!fIsInitialDownload && (chainActive.Height() % CM_BlocksPerDay) == 0))
