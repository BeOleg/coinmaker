...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (CM_UppercaseCoinName users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "CM_UppercaseCoinName " + FormatFullVersion();
...
void static StartSync(const vector<CNode*> &vNodes) {
    ...
    BOOST_FOREACH(CNode* pnode, vNodes) {
        ...
        if (!pnode->fClient && !pnode->fOneShot &&
            ...
            (pnode->nStartingHeight > (nBestHeight - CM_BlocksPerDay)) &&
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. CM_UppercaseCoinName Core is probably already running."), addrBind.ToString());
        else
