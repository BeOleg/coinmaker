...
const int CM_AllCapsCoinName_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString CM_AllCapsCoinName_IPC_PREFIX("CM_LowercaseCoinName:");
const char* CM_AllCapsCoinName_REQUEST_MIMETYPE = "application/CM_LowercaseCoinName-paymentrequest";
const char* CM_AllCapsCoinName_PAYMENTACK_MIMETYPE = "application/CM_LowercaseCoinName-paymentack";
const char* CM_AllCapsCoinName_PAYMENTACK_CONTENTTYPE = "application/CM_LowercaseCoinName-payment";
...
{
    QString name("CM_UppercaseCoinNameQt");
...
bool PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(CM_AllCapsCoinName_IPC_PREFIX, Qt::CaseInsensitive)) // CM_LowercaseCoinName: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseCM_UppercaseCoinNameURI(arg, &r))
            {
                CCM_UppercaseCoinNameAddress address(r.address.toStdString());
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    foreach (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(CM_AllCapsCoinName_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            return false;
        }
        ...
        socket->waitForBytesWritten(CM_AllCapsCoinName_IPC_CONNECT_TIMEOUT);
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click CM_LowercaseCoinName: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start CM_LowercaseCoinName: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling CM_LowercaseCoinName: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject *object, QEvent *event)
{
    // clicking on CM_LowercaseCoinName: URIs creates FileOpen events on the Mac
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in CM_LowercaseCoinName: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(CM_AllCapsCoinName_IPC_PREFIX, Qt::CaseInsensitive)) // CM_LowercaseCoinName: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseCM_UppercaseCoinNameURI(s, &recipient))
                emit receivedPaymentRequest(recipient);
            else
                emit message(tr("URI handling"),
                    tr("URI can not be parsed! This can be caused by an invalid CM_UppercaseCoinName address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach(const PAIRTYPE(CScript, qint64)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CCM_UppercaseCoinNameAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()){
            // Insecure payments to custom CM_LowercaseCoinName addresses are not supported
        ...
        if (txOut.IsDust(CTransaction::nMinRelayTxFee)) {
            QString msg = tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(CM_UppercaseCoinNameUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second));
...
void PaymentServer::fetchRequest(const QUrl& url)
{
    ...
    netRequest.setRawHeader("Accept", CM_AllCapsCoinName_REQUEST_MIMETYPE);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
    ...
    netRequest.setHeader(QNetworkRequest::ContentTypeHeader, CM_AllCapsCoinName_PAYMENTACK_CONTENTTYPE);
    netRequest.setRawHeader("User-Agent", CLIENT_NAME.c_str());
    netRequest.setRawHeader("Accept", CM_AllCapsCoinName_PAYMENTACK_MIMETYPE);
