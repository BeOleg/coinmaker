...
{
    CCM_UppercaseCoinNameAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered CM_LowercaseCoinName address / amount:
            ...
            scriptPubKey.SetDestination(CCM_UppercaseCoinNameAddress(rcp.address.toStdString()).Get());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal CM_LowercaseCoinName:URI (CM_LowercaseCoinName:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(CCM_UppercaseCoinNameAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        mapCoins[CCM_UppercaseCoinNameAddress(address).ToString().c_str()].push_back(out);
    }
}
...
{
    CTxDestination dest = CCM_UppercaseCoinNameAddress(sAddress).Get();
