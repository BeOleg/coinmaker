...
//	// Prepare to pay beneficiaries
    ...
    for (unsigned int i=0; i<coinAddressStrings.size(); i++)
    {
        ...
        // Create transaction
        CKeyID addrKeyId;
		CCM_UppercaseCoinNameAddress addr(coinAddressString);
		if(!addr.IsValid())
		{
			LogPrintf("CCM_UppercaseCoinNameAddress not valid! %s\n", coinAddressString.c_str());
			return NULL;
		}
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CM_UppercaseCoinNameMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CM_UppercaseCoinNameMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CM_UppercaseCoinNameMiner(CWallet *pwallet)
{
    LogPrintf("CM_UppercaseCoinNameMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("CM_LowercaseCoinName-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running CM_UppercaseCoinNameMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("CM_UppercaseCoinNameMiner terminated\n");
        throw;
    }
}

void GenerateCM_UppercaseCoinNames(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CM_UppercaseCoinNameMiner, pwallet));
}
