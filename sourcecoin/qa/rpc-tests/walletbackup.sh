...
CM_AllCapsCoinNameD=${1}/CM_LowercaseCoinNamed
CLI=${1}/CM_LowercaseCoinName-cli
...
B4ARGS="-datadir=$D4"
$CM_AllCapsCoinNameD $CM_AllCapsCoinNameDARGS $B4ARGS &
B4PID=$!
...
function CreateConfDir {
  DIR=$1
  mkdir -p $DIR
  CONF=$DIR/CM_LowercaseCoinName.conf
  echo "regtest=1" >> $CONF
...
# 1 block, 50 XBT each == 50 CM_AllCapsCoinCode
...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} CM_LowercaseCoinCode"
...
function StartThree {
  $CM_AllCapsCoinNameD $CM_AllCapsCoinNameDARGS $B1ARGS &
  B1PID=$!
  $CM_AllCapsCoinNameD $CM_AllCapsCoinNameDARGS $B2ARGS &
  B2PID=$!
  $CM_AllCapsCoinNameD $CM_AllCapsCoinNameDARGS $B3ARGS &
  B3PID=$!
}
