...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete CM_UppercaseCoinName database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no CM_UppercaseCoinName datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old CM_UppercaseCoinName datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected CM_UppercaseCoinName 0.7 datadir."
    ;;
  3)
    echo "Detected CM_UppercaseCoinName pre-0.8 datadir."
    ;;
  4)
    echo "Detected CM_UppercaseCoinName 0.8 datadir."
    ;;
esac
